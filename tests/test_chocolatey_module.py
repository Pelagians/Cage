...
class ChocolateyModuleUnitTests(unittest.TestCase):
    ...
    def test_chocolatey_finalizer_treats_pwsh_probe_as_hard_boundary(self):
        ...
        self.assertIn("PowerShell -Command probe did not propagate expected exit code 37", finalize)
        self.assertIn("PowerShell -Command probe did not write sentinel", finalize)
        self.assertNotIn("PowerShell -Command probe did not return expected exit code 37", finalize)
        self.assertNotIn("exit 97", finalize)
