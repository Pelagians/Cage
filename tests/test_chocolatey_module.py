...
class ChocolateyModuleUnitTests(unittest.TestCase):
    ...
    def test_chocolatey_finalizer_uses_native_pwsh_contract_like_chocinstaller(self):
        ...
        self.assertIn("unix-sentinel.txt", finalize)
        self.assertIn("[cfw-pwsh-out]", finalize)
        self.assertIn('wine "$pwsh_exe" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "$pwsh_probe_script_win"', finalize)
        self.assertIn('wine "$pwsh_exe" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "$finalize_driver_win"', finalize)
    ...
    def test_chocolatey_finalizer_captures_native_pwsh_probe_on_unix_side(self):
        ...
        self.assertIn('pwsh_probe_sentinel="$unix_probe_dir/unix-sentinel.txt"', finalize)
        self.assertIn('pwsh_probe_script="$unix_probe_dir/pwsh-probe.ps1"', finalize)
        ...
        self.assertIn('pwsh_probe_script_win="$(winepath -w "$pwsh_probe_script")"', finalize)
        self.assertIn('pwsh_probe_sentinel_win="$(winepath -w "$pwsh_probe_sentinel")"', finalize)
        ...
        self.assertNotIn('pwsh_probe_sentinel="$work_dir/pwsh-probe-ok.txt"', finalize)
