...
2. Keep pinned/checksummed upstream Chocolatey-for-wine release data, c_drive flattening, Chocolatey nupkg extraction, dedicated .NET x64-then-x86 MSI installs gated by native CLR marker presence, native CLR policy, native promotion, diagnostics, and canonical `choco.exe` package installs.
