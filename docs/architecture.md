...
`core/modules/` implements BlueBuild-style build-time modules as first-class build directives. `type: chocolatey` matches the myOS `type: dnf` YAML style: recipes declare packages under `modules[].install.packages`, while Cage lowers that intent into a pinned/checksummed upstream Chocolatey-for-wine installer run (`ChoCinstaller_*.exe /s /q`), canonical `choco.exe` diagnostics, and package install steps. `type: powershell-wrapper` is a separate module for Synchro's wrapper and is temporarily incompatible with `chocolatey` until the Phase 2 PowerShell capability resolver can reconcile shared engine, WinPS shim, and profile-library ownership.
