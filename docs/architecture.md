...
Cage compiles application recipes into immutable application artifacts for Wine/Proton-family runtimes.
...
Cage is application-first. The user-facing artifact is an application artifact; the current v0 execution bundle is a sealed filesystem staging/debug representation:
...
It includes the built prefix, runtime binding, normalized recipe/manifest, launch definition, metadata, hashes/provenance, logs, and `metadata/graph.json` as build/provenance metadata. The canonical deployable direction is an OCI image digest containing this application artifact and embedded Cage metadata.
...
`core/compatibility.py` normalizes high-level Wine/Proton compatibility intent into `cage.compatibility-policy/v0`. The builder applies WINEARCH, Windows version emulation, compatibility env, deterministic DLL override policy, and requested DXVK/vkd3d prefix backend installation. The runtime launcher and OCI app-image launcher re-export the same policy from `metadata/graph.json`, so build-time and run-time compatibility intent do not drift.
...
`core/sources.py` verifies local source presence and sha256 values before real Wine work. It emits `cage.source-integrity/v0`, resolving `file://` and relative paths against the workspace root used by the build container.

`compat/evidence.py` emits `cage.compat-test/v0`: source integrity, bundle creation, bundle verification, run-plan evidence, and optionally real build/run execution evidence. `--mode dry-run` is dependency-light. `--mode build` runs the real container build after source integrity passes. `--mode run` adds bounded app launch evidence. The evidence envelope stays stable as execution depth increases.

`compat/corpus.py` and `compat/corpus/apps.json` provide the packaged `cage.compat-corpus/v0` seed list for app testing. The corpus is a curation/input layer, not an automatic runtime selector.
...
`core/modules/` implements BlueBuild-style build-time modules as first-class build directives. `type: chocolatey` matches the myOS `type: dnf` YAML style: recipes declare packages under `modules[].install.packages`, while Cage lowers that intent into sequential, pinned/checksummed, upstream-derived Chocolatey-for-wine setup steps: PowerShell MSI compatibility state, Chocolatey-for-wine data extraction, Chocolatey nupkg extraction, .NET 4.8 x64/x86 MSIs, native-first/builtin-fallback CLR policy, native promotion into canonical `choco.exe` using the real root nupkg executable rather than the upstream redirect shim, full app-local CLR loader closure beside canonical Chocolatey, readiness diagnostics through the canonical Windows path, upstream feature policy, and Windows-path package install. `type: powershell-wrapper` is a separate module for Synchro's wrapper and is temporarily incompatible with `chocolatey` until the Phase 2 PowerShell capability resolver can reconcile shared engine, WinPS shim, and profile-library ownership.

### 8. Downloadable runner cache

`cage runners list|ensure|diagnose` exposes the runner-cache lifecycle. Diagnostics parse ELF interpreters so old 32-bit Wine builds can fail with actionable evidence such as missing `/lib/ld-linux.so.2` instead of an opaque shell error. This is required for 7040/VIC Office evidence because the Rustring/Bottles reference depends on legacy x86 Wine builds hosted by PlayOnLinux/Phoenicis.

Real build/run execution keeps runner archives host-cached but container-executed: `runtime.runner` requests are mounted read-only at `/opt/cage-runner`, `CAGE_RUNNER_BIN` is exported, and the mounted `bin/` is prepended to `PATH` inside the runtime image. Host diagnostics may show missing 32-bit loader support, but execution can still proceed inside Wine runtime images because those images install i386 Wine support.
...
`cage bundle inspect` and `cage bundle verify` form the validation layer between bundle creation and future `cage run`. Verification consumes the bundle's manifest, runtime binding, launch contract, provenance, build plan, and `metadata/graph.json` without requiring container execution.

### 11. Local artifact index

`artifact/index.py` maintains the local `cage.artifact-index/v0` cache at `dist/.cage/artifacts.json` by default. `cage build` registers verified bundles by app name and version. `cage artifacts list` and `cage artifacts resolve <name[@version]>` expose the index, and `cage run` / `cage export oci` accept either direct bundle paths or app references.
...
Exported images are based on the graph-resolved runtime image and embed the bundle at `/opt/cage/bundle`. Runtime state and exports are separate at `/var/lib/cage/state` and `/exports`.
...
`artifact/kube.py` implements `cage export kube`. It consumes a verified bundle or app-name reference and emits `cage.kube-export/v0` plus Kubernetes YAML. The emitter requires digest-pinned image refs by default and creates a Deployment plus state/export PVCs unless `--no-pvc` is set. Labels are normalized for Kubernetes selectors, while exact Cage artifact metadata is preserved in annotations.

Cage supports OCI output for distribution and Kubernetes execution as a downstream substrate, but Cage must not depend on Kubernetes internally. The Kubernetes path is manifest generation only: no namespace creation, no `kubectl apply`, no tenant/session policy, and no VIC production automation authority.
