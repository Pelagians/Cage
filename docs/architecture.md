...
`core/modules/` implements BlueBuild-style build-time modules as first-class build directives. `type: chocolatey` matches the myOS `type: dnf` YAML style: recipes declare packages under `modules[].install.packages`, while the module loads one immutable compatibility profile and emits an ordered pipeline. Cage verifies Noah's patched Chocolatey-for-Wine release and all transitive payloads, stages them in a private per-prefix work directory, invokes the fork offline, and independently gates canonical Chocolatey, readiness, feature policy, package lifecycle, and requested package installation. `type: powershell-wrapper` remains a separate, temporarily incompatible module until the Phase 2 PowerShell capability resolver can reconcile shared engine, WinPS shim, and profile-library ownership.
