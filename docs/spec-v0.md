...
Cage packages applications, not user-facing Wine prefixes. The primary user/business-authored input is a strict YAML application recipe. JSON remains supported for generated, normalized, or CLI-driven inputs.

Long-term happy path:

```bash
cage build notepad-plus-plus.cage.yaml
...
| Field | Type | Meaning |
| --- | --- | --- |
| `schemaVersion` | string | Prefer `cage.app/v0`; legacy JSON may use `cage.dev/v0` |
...
The Chocolatey module is self-contained and consumes PietJankbal's Chocolatey-for-wine release as pinned upstream data, but Cage does not trust `ChoCinstaller_*.exe` as the success boundary. Cage downloads and checksum-verifies upstream data, flattens `c_drive.7z`, extracts the pinned Chocolatey nupkg into raw `ProgramData/tools/ChocolateyInstall`, installs the upstream .NET 4.8.1 `dotnet481` manifest payload/registry data, applies upstream `mscoree=native` registry policy, promotes the raw payload into canonical `C:/ProgramData/chocolatey/bin/choco.exe` using the real root nupkg `choco.exe` rather than the upstream redirect shim, copies the full native CLR loader closure app-local beside canonical `choco.exe`, runs readiness diagnostics through the canonical Windows path, applies upstream Chocolatey feature policy, and then runs `choco install <package> -y` through that same Windows path during the online build phase. `chocolatey` and `powershell-wrapper` are temporarily incompatible in the same recipe until the Phase 2 PowerShell capability resolver lands. Package names must be non-empty strings containing only letters, numbers, dot, underscore, plus, or dash.
...
`cage run` can select a suite entrypoint and pass host files into the container as read-only input mounts:

```bash
cage run vendor-suite --entrypoint writer ./document.docx
```
...
`cage runners list` emits `cage.runner-catalog/v0` and lists downloadable runner archive aliases. The initial aliases are:
...
`cage runners ensure <alias>` downloads, verifies, and extracts the runner under the local cache, emitting `cage.runner-cache/v0`. `cage runners diagnose <alias-or-path>` emits `cage.runner-diagnostic/v0`; it detects common host/runtime blockers such as missing 32-bit ELF interpreters (`/lib/ld-linux.so.2`) before users get an opaque shell error like “cannot execute: required file not found”.

If a bundle graph contains `runnerRuntime.runner`, `cage run --runner-cache-dir <dir>` reports `runnerCache` in the `cage.run-plan/v0`. If the cached runner is present, the plan mounts the runner directory read-only at `/opt/cage-runner`, exports `CAGE_RUNNER_ID` and `CAGE_RUNNER_BIN=/opt/cage-runner/bin`, and prepends that bin directory to `PATH` before launching Wine. Dry-run planning may report `runnerCache.status: missing`; real run execution requires the runner cache to be present. Real build and compatibility evidence commands use the same cache directory to mount the selected runner into the runtime container.
...
`cage compat corpus` emits `schemaVersion: cage.compat-corpus/v0`, a packaged seed app corpus with tiers, statuses, source policies, and compatibility focus tags. The corpus is not an automatic compatibility database; it is the starter list for repeatable evidence collection.

## Artifact model

A Cage application artifact is the immutable output of a recipe-defined build. The canonical deployable direction is an OCI image digest containing the built application prefix, normalized recipe, metadata, provenance, launch contract, and runtime compatibility metadata.
...
`cage run <bundle-or-app-ref>` consumes a verified bundle, either directly by path or resolved through the local artifact index, and must fail before container planning when `cage bundle verify <bundle>` would fail.
...
`cage export oci <bundle-or-app-ref> --tag <image> --dry-run` consumes a verified bundle, either directly by path or resolved through the local artifact index, and emits a `cage.oci-export-plan/v0` document. The plan includes the graph-resolved runtime base image, application identity, requested and resolved runtime versions, OCI labels, `metadata/artifact.json` content, image layout, and generated `Containerfile` content.

`cage export oci <bundle-or-app-ref> --tag <image>` stages a build context and runs `podman build` or `docker build`. The source bundle is not mutated; export writes `metadata/artifact.json` into the staged copy.
...
Embedded artifact metadata uses `schemaVersion: cage.artifact-image/v0`. OCI labels mirror core metadata such as app name/version, runtime provider, requested/resolved runtime version, runner, launcher, and base image. OCI image digests are the deployable identity; embedded Cage metadata describes artifact semantics.

`cage export oci <bundle-or-app-ref> --tag <image> --push` pushes the image and records repo digest identity from image inspection when available. `cage image verify <image>` emits `cage.oci-image-verification/v0` and fails if OCI labels disagree with embedded `metadata/artifact.json`, if metadata cannot be read, or if the container engine cannot inspect the image.


## Kubernetes manifest export

`cage export kube <bundle-or-app-ref> --image <image@sha256:...> --dry-run` consumes a verified bundle, either directly by path or resolved through the local artifact index, and emits `cage.kube-export/v0`. The plan includes application identity, namespace, Kubernetes resource base name, PVC settings, generated resource objects, and rendered YAML.

`cage export kube <bundle-or-app-ref> --image <image@sha256:...> --output <file>` writes Kubernetes YAML. It does not apply the manifest.
...
The v0 manifest emitter creates a Deployment plus state/export PVCs by default. Runtime state mounts at `/var/lib/cage/state`; exports mount at `/exports`. `--no-pvc` replaces the PVCs with `emptyDir` volumes for smoke/demo manifests. Kubernetes labels are normalized for selector/tooling safety; exact Cage metadata such as schema, raw app name, app version, and image ref is preserved in annotations.
