...
- Cage launches manifest-declared Chocolatey queries through direct `wine` so
  their output remains observable. Package install and uninstall operations
  use the manifest-declared `wineconsole` launcher and always pass
  `--use-system-powershell`, allowing the CFW-owned Synchro wrapper to inherit
  the real Windows console required by `pwsh.exe`.
