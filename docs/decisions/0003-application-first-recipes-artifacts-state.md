...
WinForge is application-first. Users should think "I am packaging an application" rather than "I am constructing a Wine prefix." Wine, Wine Staging, Proton-GE, prefixes, launch scripts, OCI layers, and container details are provider/runtime implementation details behind an application recipe and artifact lifecycle.

The primary shareable authoring format is strict YAML application recipes using `schemaVersion: winforge.app/v0`. JSON remains supported as a normalized/generated format and for CLI-driven workflows; YAML is not the only possible build input, but it is the format different users and businesses should be able to write, store, review, and share.
...
1. **Recipe-first UX**: the long-term happy path is `winforge build app.yaml` and `winforge run app`, with lower-level bundle commands retained for debugging and automation.
...
4. **Build-time install**: WinForge-managed install/dependency/config/registry steps happen at build time.
...
7. **OCI identity, metadata semantics**: OCI digest identifies the deployable artifact; embedded WinForge metadata describes it. OCI labels should match metadata or verification should fail.
8. **Graph scope**: the execution graph is build/provenance/contract metadata, not a general runtime scheduler.

## Reasoning

This keeps the Ramalama-like experience: users select or write a recipe, WinForge resolves provider/runtime details, builds an application artifact, and then runs it with minimal user-facing complexity. Wine is only one runtime family behind the application lifecycle.

Wine/Proton prefixes are not byte-reproducible in practice because registry entries, caches, timestamps, GUIDs, fonts, shader caches, and app state mutate. WinForge should instead target functional equivalence and auditable rebuildability given the same recipe, source artifacts, WinForge version, and runtime provider image.
...
Review if WinForge adds a registry-backed application catalog, a real artifact store, OCI export as the default build output, mutable runtime snapshot/export flows, or non-Wine runtime families.
