...
Status: superseded by [0021. Upstream-first Chocolatey-for-Wine bootstrap](0021-upstream-first-chocolatey-bootstrap.md)
Date: 2026-07-09
...
6. install the upstream .NET Framework 4.8.1 `dotnet481` payload by downloading/checksum-verifying `ndp481-x86-x64-allos-enu.exe`, extracting `x64-Windows10.0-KB5011048-x64.cab`, copying manifest-described native CLR files, replacing `mscoreei.dll`, and importing manifest-derived 64-bit/32-bit registry data. Cage does this deterministically with shell/Python rather than trusting the upstream PowerShell finalizer;
...
9. copy the native CLR loader closure (`mscoree.dll`, `mscoreei.dll`, `clr.dll`, `clrjit.dll`, `ucrtbase_clr0400.dll`, `vcruntime140_clr0400.dll`) app-local beside canonical `bin/choco.exe` from the upstream `dotnet481` payload, matching upstream's `ucrtbase_clr0400.dll` readiness boundary and avoiding Wine system DLL search/dependency ambiguity;
