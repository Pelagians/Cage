...
Cage will keep downloadable runner archives in a host cache, but execute them inside the selected runtime container:

- `cage runners ensure <alias>` populates the local cache and records archive provenance.
- `cage build --runner-cache-dir <dir>` mounts the selected runner read-only at `/opt/cage-runner` during real container builds when `runtime.runner` is present.
- `cage run --runner-cache-dir <dir>` does the same for real app launches from bundles.
- `cage compat test --mode build|run --runner-cache-dir <dir>` threads the same cache through build and run evidence.
- Inside the container, Cage exports `CAGE_RUNNER_ID`, `CAGE_RUNNER_BIN=/opt/cage-runner/bin`, prepends that bin directory to `PATH`, and sets `WINE=$CAGE_RUNNER_BIN/wine`.
...
- Private Office evidence can use `runtime.runner` aliases without public Cage shipping Office recipes, containers, or payloads.
