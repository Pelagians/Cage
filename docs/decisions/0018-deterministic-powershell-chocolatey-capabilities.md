...
3. Install the upstream .NET 4.8.1 `dotnet481` payload in its own build step by host-downloading and checksum-verifying `ndp481-x86-x64-allos-enu.exe`, extracting `x64-Windows10.0-KB5011048-x64.cab`, copying manifest-described native CLR files into the prefix, replacing `mscoreei.dll`, and importing manifest-derived 64-bit/32-bit registry data. Cage owns this step with shell/Python rather than treating upstream PowerShell execution as a trusted success boundary, and the native CLR marker files must exist before Chocolatey verification.
...
- It starts four unsynchronized threads: `c_drive.7z` extraction, upstream .NET payload extraction, PowerShell MSI via Wine URLMon/WinINet, and Chocolatey nupkg extraction.
