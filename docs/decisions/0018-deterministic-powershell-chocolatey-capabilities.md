...
4. Prepare Wine registry/runtime state: set the prefix Windows version to `win10`, apply native-first/builtin-fallback `mscoree` policy for Chocolatey, and apply the `pwsh.exe` DLL overrides required by Chocolatey-for-wine.
