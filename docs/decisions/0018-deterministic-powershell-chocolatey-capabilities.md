...
3. Install .NET 4.8 in its own build step by host-downloading the pinned installer, extracting both `netfx_Full_x86.msi` and `netfx_Full_x64.msi`, and running sequential dedicated `msiexec /QN` installs with generous timeouts. This step must never run in parallel with another MSI operation, and both WOW64 and 64-bit native CLR files must exist before Chocolatey verification.
...
5. Promote Chocolatey natively: copy the raw `C:/ProgramData/tools/ChocolateyInstall` payload to canonical `C:/ProgramData/chocolatey`, create canonical `bin` redirects, and apply Chocolatey environment registry values with native shell/Python operations. Do not run upstream `choc_install.ps1` through `pwsh.exe` as the success boundary in this runtime.
6. Verify only canonical `C:/ProgramData/chocolatey/bin/choco.exe`; the raw `tools/ChocolateyInstall` payload is a source/recovery marker, never success.
