...
5. Promote Chocolatey natively: copy the raw `C:/ProgramData/tools/ChocolateyInstall` payload to canonical `C:/ProgramData/chocolatey`, place the real root nupkg `choco.exe` into canonical `bin/choco.exe` instead of the upstream redirect shim, keep helper redirects such as `RefreshEnv.cmd`, copy native CLR loader dependencies app-local beside canonical Chocolatey, and apply Chocolatey environment registry values with native shell/Python operations. Do not run upstream `choc_install.ps1` through `pwsh.exe` as the success boundary in this runtime.
