...
- Gate `cage-wine:11.0` CI with `container/common/cage-powershell-runtime-smoke.sh`, which follows Cage's prefix lifecycle (`mscoree,mshtml=` during `wineboot`, cleared before PowerShell), installs the pinned PowerShell MSI in a fresh prefix, applies the `pwsh.exe` AppDefaults DLL overrides (`amsi=`, `dwmapi=`, `rpcrt4=native,builtin`), and requires `PWSH-ALIVE` plus sentinel evidence through direct, nested `cmd`, or generated C-drive `.cmd` launcher modes before Chocolatey work resumes. Smoke failure annotations include base64 stdout/stderr previews because raw Actions logs can be hidden.
