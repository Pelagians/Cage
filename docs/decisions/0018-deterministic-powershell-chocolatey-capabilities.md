...
5. Prove the finalization PowerShell boundary before finalization. A `pwsh.exe` file and Wine exit code `0` are not sufficient. Inline `pwsh.exe -Command` and direct ZIP `pwsh.exe -File` are not authoritative for Chocolatey-for-wine; Cage must verify stdout plus sentinel creation through the pinned Chocolatey-for-wine `WindowsPowerShell/v1.0/powershell.exe` wrapper/profile script-file path. Because that wrapper launches bare `pwsh`, Cage must expose the ZIP-installed PowerShell directory to the wrapper with Windows-form `WINEPATH`, keep Unix `PATH` as host-side fallback, and seed the upstream `PS7` environment value.
