...
5. Prove PowerShell execution before finalization. A `pwsh.exe` file and Wine exit code `0` are not sufficient; Cage must verify stdout plus sentinel creation through both `-Command` and script-file invocation. Exit-code propagation is useful diagnostic signal but is not authoritative because Wine can return `0` even after PowerShell runs `exit 37`.
