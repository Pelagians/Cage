...
- Gate `cage-wine:11.0` CI with `container/common/cage-powershell-runtime-smoke.sh`, which follows Cage's prefix lifecycle (`mscoree,mshtml=` during `wineboot`, cleared before PowerShell), installs the pinned PowerShell MSI in a fresh prefix, applies the `pwsh.exe` AppDefaults DLL overrides (`amsi=`, `dwmapi=`, `rpcrt4=native,builtin`) as a captured/annotated boundary, and requires `PWSH-ALIVE` plus sentinel evidence through direct, nested `cmd`, or generated C-drive `.cmd` launcher modes before Chocolatey work resumes. Smoke failure annotations include base64 stdout/stderr previews because raw Actions logs can be hidden. When this gate fails, run non-blocking comparison smokes on adjacent Wine images (`wine:10.0`, `staging:11.0`, `staging:11.10`) before adding low-level Wine/CoreCLR diagnostics. If all adjacent images fail with the same silent no-op pattern, capture a bounded `WINEDEBUG=+loaddll,+seh` probe around `pwsh.exe` before the final smoke failure, and include a bounded `wineserver -w` settle after PowerShell launches so Wine-spawned child processes can create side effects before sentinel checks.
